...
export const Dashboard = () => {
  ...
  const totalDonations = useMemo(() => donations.reduce((sum, d) => sum + Number(d.amount || 0), 0), [donations]);
  const upcomingEvents = useMemo(() => events.filter(e => e.date ? new Date(e.date) >= new Date() : true).length, [events]);
  ...
  const recentDonations = useMemo(() => {
    if (isDonationsEmpty) {
      return [
        { id: 1, donor: 'Michael Johnson', amount: 1500, date: '2023-10-12', campaign: 'Winter Relief' },
        { id: 2, donor: 'Sarah Williams', amount: 250, date: '2023-10-11', campaign: 'Education Fund' },
        { id: 3, donor: 'Global Tech LLC', amount: 5000, date: '2023-10-09', campaign: 'General Fund' },
        { id: 4, donor: 'David Brown', amount: 100, date: '2023-10-08', campaign: 'Food Drive' },
        { id: 5, donor: 'Anonymous', amount: 500, date: '2023-10-05', campaign: 'Winter Relief' },
      ];
    }
    return [...donations]
  ...
  const StatCard = ({ title, value, subtitle, icon, colorClass }) => (
    <div className="bg-white dark:bg-gray-800 p-4 sm:p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 flex items-center gap-4 sm:gap-6 hover:shadow-md dark:shadow-black/50 transition-shadow">
      <div className={`w-12 h-12 sm:w-16 sm:h-16 rounded-full flex items-center justify-center ${colorClass} bg-opacity-10 dark:bg-opacity-20 shrink-0`}>
        {icon}
      </div>
      <div className="min-w-0">
        <p className="text-xs sm:text-sm font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1 truncate">{title}</p>
        <h3 className="text-xl sm:text-2xl md:text-3xl font-bold text-gray-900  truncate">{value}</h3>
        {subtitle && <p className="text-[10px] sm:text-xs text-gray-400 dark:text-gray-500 mt-1 truncate">{subtitle}</p>}
      </div>
    </div>
  );
  ...
  return (
    <div className="w-full flex flex-col gap-8 animate-fade-in-up pb-8 mt-2">
      ...
      {/* Header Profile */}
      <div>
        <h1 className="text-2xl sm:text-3xl font-extrabold text-trust-blue dark:text-blue-400">Organization Dashboard</h1>
        <p className="text-sm sm:text-base text-gray-500 dark:text-gray-400 mt-1 sm:mt-2">Welcome back! Here is a high-level overview of your operations.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatCard 
          ...
          subtitle={isDonationsEmpty ? '+14% from last month' : 'Lifetime aggregation'}
          colorClass="bg-trust-gold text-trust-gold"
          icon={(
             <svg fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-6 h-6 sm:w-8 sm:h-8">
        ...
        <StatCard 
          ...
          value={upcomingEvents === 0 && events.length === 0 ? '3' : upcomingEvents}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white dark:bg-gray-800 p-4 sm:p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 min-w-0">
          <h2 className="text-base sm:text-lg font-bold text-gray-800  mb-4 sm:mb-6">Donation Trends</h2>
          <div className="h-[250px] sm:h-[300px] w-full min-w-0">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={monthlyDonations} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                ...
                <Tooltip 
                  ...
                  contentStyle={{ borderRadius: '12px', border: 'none', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)', backgroundColor: theme === 'dark' ? '#1F2937' : '#fff', color: theme === 'dark' ? '#fff' : '#000' }}
                />
                <Bar dataKey="amount" fill="#1e3a5f" radius={[6, 6, 0, 0]} maxBarSize={60} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 p-4 sm:p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 flex flex-col min-w-0">
          <h2 className="text-base sm:text-lg font-bold text-gray-800  mb-2">Category Demographics</h2>
          <div className="h-[250px] sm:h-[300px] w-full min-w-0">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden min-w-0">
        <div className="px-4 sm:px-6 py-4 sm:py-5 border-b border-gray-100 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-900/50 flex flex-col sm:flex-row sm:items-center justify-between gap-1 sm:gap-4">
          <h2 className="text-base sm:text-lg font-bold text-gray-800 dark:text-white">Recent Donations</h2>
          <span className="text-xs sm:text-sm text-gray-500 dark:text-gray-400 font-medium whitespace-nowrap">Last 5 records</span>
        </div>
        <div className="overflow-x-auto">
        <table className="w-full text-left border-collapse min-w-[600px]">
  <thead>
    <tr className="bg-white dark:bg-gray-800 text-gray-400 dark:text-gray-300 text-[10px] sm:text-xs uppercase tracking-wider">
